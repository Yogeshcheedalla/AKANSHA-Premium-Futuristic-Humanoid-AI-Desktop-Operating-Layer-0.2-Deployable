 ...
 * Returns candidate models classified against the REAL device + runtime. This is a
 * capability plugged under the existing Model Center; it does not replace it and does
 * not perform installs (that stays with ModelManager/RuntimeManager). INSTALL is only
 * flagged when a supported format + runtime actually exist — never faked.
 */
export async function GET(request: Request) {
  ...
  try {
    ...
    const results = await discoverAndRank(q, hardware, rt.available, { limit: 12 });
